...
{      			    
	...
	OLED_SetCursor(x,y);	//从 x y 开始画点，先画第一页
    for(t=0;t<16;t++) //循环16次，画第一页的16列
		{
			OLED_I2C_SendByte(Chn[2*no][t]);//画no在数组位置的第一页16列的点
			adder+=1; //数组地址加1
     	}	
		OLED_SetCursor(x,y+1); //画第二页
    for(t=0;t<16;t++)//循环16次，画第二页的16列
		{	
			OLED_I2C_SendByte(Chn[2*no+1][t]);//画no在数组位置的第二页16列的点
			adder+=1;//数组地址加1
        }					
}
